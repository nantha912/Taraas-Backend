...
/**
 * ProviderService - Refined for High-Scale Production Incremental Math
 * Implementation of the 4-Scenario Search Capability without relational DB joins.
 */
@Service
public class ProviderService {
    ...
    public List<ProviderSearchResultDTO> searchWithRanking(String service, Double lat, Double lon, String city, String mode) {
        ...
        // 👑 HIGH-SCALE OPTIMIZATION TRICK:
        // Relational data joins and manual loop mappings are COMPLETELY removed here.
        // We evaluate weighted ranking using the provider's native fields instantly!
        ...
        Document scoringFormula = isRemoteMode ?
                ...
                new Document("$add", Arrays.asList(
                        ...
                        new Document("$min", Arrays.asList(new Document("$multiply", Arrays.asList("$completedOrdersCount", 0.3)), 30)),
                        20
                )) :
                // Nearby Priority: Proximity (30) + Trust (30) + Volume (20)
                new Document("$add", Arrays.asList(
                        new Document("$multiply", Arrays.asList("$averageRating", 6)),
                        new Document("$min", Arrays.asList(new Document("$multiply", Arrays.asList("$completedOrdersCount", 0.2)), 20)),
        ...
        // --- CLEAN ID STRUCT PROJECTION MAP ---
